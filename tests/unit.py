...
check("AURACORE" in sp, "Mentions AURACORE")
check("segundo cerebro" in sp, "Mentions 'segundo cerebro' purpose")
check("NO hacés" in sp.lower() or "NUNCA hacés" in sp, "Defines limits explicitly")
check("vehículos" in sp, "Mentions vehicles entity")
check("documentos" in sp, "Mentions documents entity")
check("eventos" in sp or "recordatorios" in sp, "Mentions events/reminders")
check("listas" in sp, "Mentions lists entity")
check("notas" in sp, "Mentions notes entity")
check("proyectos" in sp.lower(), "Mentions projects entity")
check("gastos compartidos" in sp.lower(), "Mentions shared expenses")
check("contactos" in sp.lower(), "Mentions contacts entity")
check("pico y placa" in sp, "Mentions pico y placa (Ecuador context)")
check("confirmá" in sp or "confirmá lo que entendiste" in sp, "Rule: confirm before saving")
...
check(len(LUCHO_SYSTEM_PROMPT_SHORT) < 500, f"Short prompt is short ({len(LUCHO_SYSTEM_PROMPT_SHORT)} chars)")
